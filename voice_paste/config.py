...
# --- AI送信設定 ---
# AI_SEND_1_NAME / AI_SEND_1_URL, AI_SEND_2_NAME / AI_SEND_2_URL, ... で複数設定可
AI_SEND_DELAY: float = float(os.getenv("AI_SEND_DELAY", "2.5"))
AI_SEND_HOTKEY: str = os.getenv("AI_SEND_HOTKEY", "<ctrl>+<alt>+a")
...
while True:
    ...
    if not _name or not _url:
        break
    AI_SEND_APPS.append({"name": _name, "url": _url})
    _ai_idx += 1
